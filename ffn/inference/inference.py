...
class Runner(object):
  ...
  def start(self, request, batch_size=1, exec_cls=None, session=None):
    ...
    if not os.path.exists(request.segmentation_output_dir):
      os.makedirs(request.segmentation_output_dir)
    ...
    if request.reference_histogram:
      with os.open(request.reference_histogram, 'r') as f:
        data = np.load(f)
    ...
    # todo dw
    # self.model = torch.load(request.model_name)
    self.model = ffn()
    ...
    self.executor = exec_cls(
        self.model, self.counters, batch_size)
    self.movement_policy_fn = movement.get_policy_fn(request, self.model)

    self.model.load_state_dict(torch.load(request.model_checkpoint_path))
