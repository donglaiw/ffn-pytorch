...
def atomic_file(path, mode='w+b'):
  ...
  with tempfile.NamedTemporaryFile(mode=mode) as tmp:
    ...
    shutil.copy(tmp.name, '%s.tmp' % path, overwrite=True)
  shutil.move('%s.tmp' % path, path, overwrite=True)
...
def get_existing_corners(segmentation_dir):
  corners = []
  # Legacy path format.
  for path in glob.glob(os.path.join(segmentation_dir, 'seg-*_*_*.npz')):
...
def load_origins(segmentation_dir, corner):
  ...
  with os.open(target_path, 'r') as f:
    data = np.load(f)
